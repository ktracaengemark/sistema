(function($) {    
    ...
    function formatTime(hour, minute)
    {
        ...
        if (printHour < 10) printHour = '0' + printHour;        
    ...
    function renderCalendarPage(element, year, month, options)
    {
        ...
        if (options.addPrevAndPostWeek && ff < 6) ff += 7;
        ...
        var rewind = (options.addPrevAndPostWeek) ? date.getDay() + 7 : date.getDay();
    ...
    function renderTimeSelect(element, options)
    {
        ...
        //alert (options.selection);
        var ul = $('<ul />');
        for (var hour = 7; hour < 24; hour++) {
            for (var minute = 0; minute < 60; minute += 30) {
                if (startTime && startTime > ((hour * 60 + minute)-30)) continue;
    ...
    $.fn.calendricalDate = function(options)
    {
        ...
        return this.each(function() {
            ...
            element.bind('focus click', function() {
                if (div) return;
                var offset = options.relativePositioning ? element.position() : element.offset();

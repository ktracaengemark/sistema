<div class="container-fluid">
	<div class="row">
		...
		<div class="col-lg-1 col-md-2 col-sm-2 col-xs-6 text-left">
			<label></label><br>
			<a href="<?php echo base_url() . 'gerar_excel/Marketing/Marketing_total_xls.php'; ?>">
						...
						foreach ($report->result_array() as $row) {
							...
							echo '<tr>';
							 ...
							 /*
							  ...
							  exit();
								*/	
								//echo '<tr class="clickable-row" data-href="' . base_url() . 'marketing/alterar/' . $row['idApp_Marketing'] . '">';
								
								echo '<td>' . ($linha + $count) . '</td>';
								echo '<td>' . $row['NomeUsuario'] . '</td>';
								echo '<td>' . $row['DataMarketing'] . '</td>';
								echo '<td>' . $row['HoraMarketing'] . '</td>';
								if($query['TipoMarketing'] == 1 || $query['TipoMarketing'] == 2) {
								...
								}elseif($query['TipoMarketing'] == 3 || $query['TipoMarketing'] == 4) {
									if($query['TipoMarketing'] == 3){
										echo '<td class="notclickable">
												<a class="btn btn-md btn-' . $panel . ' notclickable" href="' . base_url() . 'marketing/tela_'.$titulo1.'/' . $row['idApp_Marketing'] . '">
													 ' . $row['idApp_Marketing'] . '
												</a>
											</td>';
									}elseif($query['TipoMarketing'] == 4){
										echo '<td class="notclickable">
												<a class="btn btn-md btn-' . $panel . ' notclickable" href="' . base_url() . 'marketing/tela_'.$titulo1.'/' . $row['idApp_Marketing'] . '">
													 ' . $row['idApp_Marketing'] . '
												</a>
								...
								if($query['TipoMarketing'] == 3 || $query['TipoMarketing'] == 4) {
								...
								if($nome == "Cliente"){	
									if(isset($row['idApp_' . $nome]) && $row['idApp_' . $nome] != 0){	
										if(isset($row['idApp_OrcaTrata']) && $row['idApp_OrcaTrata'] != 0){
											...
											if($query['TipoMarketing'] == 3){
												echo '<td class="notclickable">
														<a class="btn btn-md btn-info notclickable" href="' . base_url() . 'marketing/listar_Marketing/' . $row['idApp_' . $nome] . '">
															 ' . $row['idApp_' . $nome] . '
														</a>
													</td>';
											}elseif($query['TipoMarketing'] == 4){	
								...
								/*
								echo '<td class="notclickable">
										<a class="btn btn-md btn-info notclickable" href="' . base_url() . 'marketing/alterar/' . $row['idApp_Marketing'] . '">
											 ' . $row['idApp_Marketing'] . '
										</a>
									</td>';
									*/
								echo '<td>' . $row['Marketing'] . '</td>';	
								echo '<td>' . $row['NomeCompartilhar'] . '</td>';					
								echo '<td>' . $row['ConcluidoMarketing'] . '</td>';
								if($query['TipoMarketing'] == 3 || $query['TipoMarketing'] == 4) {
									echo '<td>' . $row['SubMarketing'] . '</td>';
									echo '<td>' . $row['DataSubMarketing'] . '</td>';
									echo '<td>' . $row['HoraSubMarketing'] . '</td>';
									echo '<td>' . $row['NomeSubUsuario'] . '</td>';							
									echo '<td>' . $row['ConcluidoSubMarketing'] . '</td>';
								}

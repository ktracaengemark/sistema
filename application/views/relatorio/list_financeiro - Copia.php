<div class="panel panel-default">
    <div class="panel-body">
        ...
        <div class="col-md-2">
            <label for="DataFim">Total Entrada:</label>
            <div class="input-group">
                <span class="input-group-addon">R$</span>
	...
	<div class="panel-body">
		...
		<div class="col-md-2">
            ...
            <div class="input-group">
                <span class="input-group-addon">R$</span>
                <input type="text" class="form-control" disabled aria-label="Total a Pagar" value="<?php echo $report->soma->somapagar ?>">
            </div>
        </div>
        ...
        <div class="col-md-2">
            <label for="DataFim">Total Saída:</label>
            <div class="input-group">
                <span class="input-group-addon">R$</span>
                <input type="text" class="form-control" disabled aria-label="Total Saída" value="<?php echo $report->soma->balanco2 ?>">
            </div>
        </div>
            ...
            <table class="table table-bordered table-condensed table-hover">
                ...
                <tbody>
                    ...
                    foreach ($report->result_array() as $row) {
                        ...
                        #echo '<tr>';
                        echo '<tr class="clickable-row" data-href="' . base_url() . 'despesas/alterar/' . $row['idApp_OrcaTrata'] . '">';
                           # echo '<td>' . $row['idApp_OrcaTrata'] . '</td>';
                           # echo '<td>' . $row['NomeCliente'] . '</td>';                           
                           # echo '<td>' . $row['DataOrca'] . '</td>';
                           # echo '<td>' . $row['DataEntradaOrca'] . '</td>';
                           # echo '<td class="text-right">R$ ' . $row['ValorEntradaOrca'] . '</td>';
							...
							echo '<td class="text-right">R$ ' . $row['ValorParcelaPagaveis'] . '</td>';
                           # echo '<td>' . $row['DataPagoRecebiveis'] . '</td>';

							echo '<td class="text-right">R$ ' . $row['ValorPagoPagaveis'] . '</td>';
                            echo '<td>' . $row['QuitadoRecebiveis'] . '</td>';
                        echo '</tr>';
                    }
                    ?>

...
class Orcatrata_model extends CI_Model {
	...
	public function get_baixadasreceitas($data, $total = FALSE, $limit = FALSE, $start = FALSE, $date = FALSE) {
		...
		if($data){
			...
			$groupby = ($data['Agrupar'] != "0") ? 'GROUP BY OT.' . $data['Agrupar'] . '' : 'GROUP BY OT.idApp_OrcaTrata';
		...
		if($total == TRUE && $date == FALSE) {
			...
			$query_total = $this->db->query(
				...
				WHERE
					...
					OT.id_Funcionario != 0 AND
					OT.CanceladoOrca = "N" AND
					OT.StatusComissaoOrca = "N" AND
					OT.id_Comissao = 0 
					' . $orcamento . '
					' . $id_comissao . '
					' . $cliente . '
    ...
    public function get_baixadacomissaoass($data, $total = FALSE, $limit = FALSE, $start = FALSE, $date = FALSE) {
    ...
    public function get_baixadacomissaofunc($data, $total = FALSE, $limit = FALSE, $start = FALSE, $date = FALSE) {
		...
		$orcamento = ($data['Orcamento']) ? ' AND OT.idApp_OrcaTrata = ' . $data['Orcamento'] . '  ': FALSE;
		$id_recibo = ($data['id_ComissaoFunc']) ? ' AND OT.id_ComissaoFunc = ' . $data['id_ComissaoFunc'] . '  ': FALSE;
		$cliente = ($data['Cliente']) ? ' AND OT.idApp_Cliente = ' . $data['Cliente'] . '' : FALSE;
		$id_cliente = ($data['idApp_Cliente']) ? ' AND OT.idApp_Cliente = ' . $data['idApp_Cliente'] . '' : FALSE;
		$tipofinandeiro = ($data['TipoFinanceiro']) ? ' AND OT.TipoFinanceiro = ' . $data['TipoFinanceiro'] : FALSE;
		$idtipord = ($data['idTab_TipoRD']) ? ' AND OT.idTab_TipoRD = ' . $data['idTab_TipoRD'] : ' AND OT.idTab_TipoRD = 2';
		...
		if($total == TRUE && $date == FALSE) {
			...
			$query_total = $this->db->query(
				...
				WHERE
					...
					OT.idSis_Empresa= ' . $_SESSION['log']['idSis_Empresa'] . ' AND
					OT.CanceladoOrca = "N" AND
					OT.StatusComissaoFunc = "N" AND
					OT.id_ComissaoFunc = 0 
					' . $orcamento . '
					' . $id_recibo . '
					' . $cliente . '
					' . $id_cliente . '
    ...
    public function get_baixadascobrancas($data, $total = FALSE, $limit = FALSE, $start = FALSE, $date = FALSE) {
    ...
    public function get_baixadosdebitos($data, $total = FALSE, $limit = FALSE, $start = FALSE, $date = FALSE) {

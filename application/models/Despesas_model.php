...
class Despesas_model extends CI_Model {
	...
	public function list_procedimentos($data = FALSE, $completo = FALSE, $total = FALSE, $limit = FALSE, $start = FALSE, $date = FALSE) {
			...
			$Fornecedor = ($data['Fornecedor']) ? ' AND PRC.idApp_Fornecedor = ' . $data['Fornecedor'] . '' : FALSE;
			$idApp_Fornecedor = ($data['idApp_Fornecedor']) ? ' AND PRC.idApp_Fornecedor = ' . $data['idApp_Fornecedor'] . '' : FALSE;  		
			...
			$query = $this->db->query('
				...
				FROM
					App_Procedimento AS PRC
						...
						LEFT JOIN App_Fornecedor AS C ON C.idApp_Fornecedor = PRC.idApp_Fornecedor

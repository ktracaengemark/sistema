...
class Relatorio_model extends CI_Model {
    ...
    public function list_comissao($data = FALSE, $completo, $total = FALSE, $limit = FALSE, $start = FALSE, $date = FALSE) {
		...
		*/
		$query = $this->db->query('
            SELECT
				...
				CONCAT(IFNULL(US.idSis_Usuario,""), " - " ,IFNULL(US.Nome,"")) AS NomeColaborador,
				USA.Nome,
				CONCAT(IFNULL(USA.idSis_Associado,""), " - " ,IFNULL(USA.Nome,"")) AS NomeAssociado,
				MD.Modalidade,
				VP.Abrev2,
				VP.AVAP,
				TFP.FormaPag,
            ...
            FROM
				...
				LEFT JOIN Sis_Empresa AS EMP ON EMP.idSis_Empresa = OT.idSis_Empresa
				LEFT JOIN Sis_Usuario AS US ON US.idSis_Usuario = OT.idSis_Usuario
				LEFT JOIN Sis_Associado AS USA ON USA.idSis_Associado = OT.Associado
				LEFT JOIN Tab_TipoFrete AS TTF ON TTF.idTab_TipoFrete = OT.TipoFrete
            WHERE
                ' . $date_inicio_orca . '
                ' . $date_fim_orca . '
...
*/		
    ...
    public function select_cliente_associado() {
        ...
        $query = $this->db->query('
            SELECT
                ASS.idSis_Associado,
                CONCAT(IFNULL(ASS.idSis_Associado, ""), " - ", IFNULL(C.NomeCliente, "")) As NomeAssociado
            FROM
	...
	public function select_usuario() {
        ...
        $query = $this->db->query('
            SELECT
				P.idSis_Usuario,
				CONCAT(IFNULL(P.idSis_Usuario, ""), " - ", IFNULL(P.Nome,"")) AS NomeUsuario
            FROM
                Sis_Usuario AS P
					LEFT JOIN Tab_Funcao AS F ON F.idTab_Funcao = P.Funcao
            WHERE

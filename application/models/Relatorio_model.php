...
exit();*/
	...
	public function list_rankingvendas($data) {
        ...
        foreach ($query['NomeCliente'] as $row) {
            #echo $row->idTab_Produtos . ' # ' . $row->Produtos . '<br />';
            #$estoque[$row->idTab_Produtos] = $row->Produtos;
            $rankingvendas[$row->idApp_Cliente] = array();
        ...
        foreach ($rankingvendas as $key => $row) {
            if(isset($row['QtdOrcam']) && isset($row['QtdDescon']))
                $rankingvendas[$key]['QtdVendida'] = $row['QtdOrcam'] - $row['QtdDescon'];
            elseif(isset($row['QtdOrcam']) && !isset($row['QtdDescon']))
                $rankingvendas[$key]['QtdVendida'] = $row['QtdOrcam'];
            elseif(!isset($row['QtdOrcam']) && isset($row['QtdDescon']))
                $rankingvendas[$key]['QtdVendida'] = $row['QtdDescon'];
            else
                $rankingvendas[$key]['QtdVendida'] = 0;
        }

        if(isset($data['CampoArray'])) {
            foreach ($rankingvendas as $key => $row)
                $campo[$key] = (isset($row[substr($data['CampoArray'], 1)])) ? $row[substr($data['CampoArray'], 1)] : 0;

            ($data['Ordenamento'] == 'ASC') ? array_multisort($campo, SORT_ASC, $rankingvendas) :  array_multisort($campo, SORT_DESC, $rankingvendas);
        }

		$rankingvendas['soma'] = array();
		...
		foreach ($rankingvendas as $key => $row) {
            ...
            $row['QtdVendida'] = (!isset($row['QtdVendida'])) ? 0 : $row['QtdVendida'];

            if ($row['QtdOrcam'] == 0 && $row['QtdDescon'] == 0 && $row['QtdParc'] == 0 && $row['QtdDevol'] == 0 && $row['QtdVendida'] == 0) {
                unset($rankingvendas[$key]);
                continue;
            }

			$somaqtdorcam += $row['QtdOrcam'];
            $rankingvendas[$key]['QtdOrcam'] = number_format($row['QtdOrcam'], 2, ',', '.');

			$somaqtddescon += $row['QtdDescon'];
            $rankingvendas[$key]['QtdDescon'] = number_format($row['QtdDescon'], 2, ',', '.');

			$somaqtdvendida += $row['QtdVendida'];
            $rankingvendas[$key]['QtdVendida'] = number_format($row['QtdVendida'], 2, ',', '.');

			$somaqtdparc += $row['QtdParc'];
            $rankingvendas[$key]['QtdParc'] = number_format($row['QtdParc'], 2, ',', '.');

			$somaqtddevol += $row['QtdDevol'];
            $rankingvendas[$key]['QtdDevol'] = number_format($row['QtdDevol'], 2, ',', '.');

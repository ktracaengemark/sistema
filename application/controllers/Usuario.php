...
class Usuario extends CI_Controller {
    ...
    public function prontuario($id) {
        ...
        $_SESSION['QueryUsuario'] = $data['query'] = $this->Usuario_model->get_usuario($id, TRUE);
    ...
    public function atuacoes($id = FALSE) {
        ...
        if ($id) {
            $_SESSION['QueryUsuario'] = $data['query'] = $this->Usuario_model->get_usuario($id);
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if (isset($data['funcao']) || (!isset($data['funcao']) && isset($data['update']['funcao']['anterior']) ) ) {
                ...
                for($j=0;$j<$max;$j++) {
                    $data['update']['funcao']['inserir'][$j]['idSis_Empresa'] = $_SESSION['QueryUsuario']['idSis_Empresa'];

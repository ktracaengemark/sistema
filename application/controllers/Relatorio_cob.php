...
class Relatorio_cob extends CI_Controller {
	...
	public function cobrancas() {
		...
		*/
        ...
        if ($this->form_validation->run() !== FALSE) {
			...
			$data['pesquisa_query'] = $this->Relatorio_model_list->list_cobrancas($_SESSION['FiltroCobrancas'],TRUE, TRUE);
			...
			if($data['pesquisa_query'] === FALSE){
				...
				$data['report'] = $this->Relatorio_model_list->list_cobrancas($_SESSION['FiltroCobrancas'], TRUE, FALSE, $config['per_page'], $data['linha']);			
	...
	public function cobrancas_excel($id = FALSE) {
		...
		$data['report'] = $this->Relatorio_model_list->list_cobrancas($dados, TRUE, FALSE, FALSE, FALSE);
    ...
    public function cobrancas_lista($id = FALSE) {
		...
		if (!$id) {
			...
			if($_SESSION['log']['idSis_Empresa'] !== $id){
				...
				$data['pesquisa_query'] = $this->Relatorio_model_list->list_cobrancas($_SESSION['FiltroCobrancas'], TRUE, TRUE, FALSE, FALSE, FALSE);
				...
				if($data['pesquisa_query'] === FALSE){
					...
					#### App_OrcaTrata ####
					$data['orcatrata'] = $this->Relatorio_model_list->list_cobrancas($_SESSION['FiltroCobrancas'], TRUE, FALSE, $config['per_page'], ($page * $config['per_page']), TRUE);
					/*
					echo '<br>';
					echo "<pre>";
		...
		if($_SESSION['Usuario']['Nivel'] == 2){
			...
			if ($id) {
				...
				if($data['empresa'] === FALSE){
					...
					$data['pesquisa_query'] = $this->Relatorio_model_list->list_cobrancas($_SESSION['FiltroCobrancas'], FALSE, TRUE, FALSE, FALSE, FALSE);
					...
					if($data['pesquisa_query'] === FALSE){
						...
						#### App_Parcelas ####
						$data['parcelasrec'] = $this->Relatorio_model_list->list_cobrancas($_SESSION['FiltroCobrancas'], FALSE, TRUE, $_SESSION['FiltroCobrancas']['Per_Page'], ($_SESSION['FiltroCobrancas']['Pagina'] * $_SESSION['FiltroCobrancas']['Per_Page']), TRUE);
			...
			if(!$data['empresa']['idSis_Empresa'] || $data['empresa']['idSis_Empresa'] !== $_SESSION['log']['idSis_Empresa']){
				...
				if ($this->form_validation->run() === FALSE) {
					...
					if($this->Basico_model->get_dt_validade() === FALSE){
						...
						#### App_ParcelasRec ####
						$data['update']['parcelasrec']['anterior'] = $this->Relatorio_model_list->list_cobrancas($_SESSION['FiltroCobrancas'], FALSE, TRUE, $_SESSION['FiltroCobrancas']['Per_Page'], ($_SESSION['FiltroCobrancas']['Pagina'] * $_SESSION['FiltroCobrancas']['Per_Page']), TRUE);

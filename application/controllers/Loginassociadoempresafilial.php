...
#controlador de Loginassociadoempresafilial

defined('BASEPATH') OR exit('No direct script access allowed');

class Loginassociadoempresafilial extends CI_Controller {

    public function __construct() {
        parent::__construct();

        $this->load->model(array('Loginassociadoempresafilial_model', 'Basico_model'));
        $this->load->helper(array('form', 'url'));
        $this->load->library(array('basico', 'form_validation', 'user_agent'));
        $this->load->driver('session');

        #load header view
        $this->load->view('basico/headerloginassociadoempresafilial');
    ...
    public function index() {
        ...
        elseif ($this->input->get('m') == 3)
            $data['msg'] = $this->basico->msg('<strong>Sua sessão expirou. Faça o loginassociadoempresafilial novamente.</strong>', 'erro', TRUE, TRUE, TRUE);
        elseif ($this->input->get('m') == 4)
            $data['msg'] = $this->basico->msg('<strong>Usuário ativado com sucesso! Faça o loginassociadoempresafilial para acessar o sistema.</strong>', 'sucesso', TRUE, TRUE, TRUE);
        ...
        if ($this->form_validation->run() === FALSE) {
            #load loginassociadoempresafilial view
            $this->load->view('loginassociadoempresafilial/form_loginassociadoempresafilial', $data);
        } else {
            ...
            /*
              ...
              exit();
             */
            $query = $this->Loginassociadoempresafilial_model->check_dados_usuario($senha, $usuario, TRUE);
            #$_SESSION['log']['Agenda'] = $this->Loginassociadoempresafilial_model->get_agenda_padrao($query['idSis_EmpresaFilial']);
            ...
            if ($query === FALSE) {
                ...
                $this->load->view('form_loginassociadoempresafilial', $data);
                ...
                if ($this->Loginassociadoempresafilial_model->set_acesso($_SESSION['log']['id'], 'LOGIN') === FALSE) {
                    $msg = "<strong>Erro no Banco de dados. Entre em contato com o Administrador.</strong>";

                    $this->basico->erro($msg);
                    $this->load->view('form_loginassociadoempresafilial');
                } else {
        ...
        #load footer view
        $this->load->view('basico/footerloginassociadoempresafilial');
    ...
    public function registrar() {
        ...
        if ($this->form_validation->run() === FALSE) {
            #load loginassociadoempresafilial view
            $this->load->view('loginassociadoempresafilial/form_registrar', $data);
        } else {
            ...
            $data['idSis_EmpresaFilial'] = $this->Loginassociadoempresafilial_model->set_usuario($data['query']);
            $_SESSION['log']['id'] = 1;
            ...
            if ($data['idSis_EmpresaFilial'] === FALSE) {
                ...
                /*
                  ...
                  . 'http://www.romati.com.br/app/loginassociadoempresafilial/confirmar/' . $data['query']['Codigo']);
                ...
                $data['aviso'] = ''
                  ...
                  <h4>
                  <p><b>Usuário cadastrado com sucesso!</b></p>
                  <p>Clique no botão abaixo e retorne para a tela de loginassociadoempresafilial para entrar no sistema.</p>
                  </h4>
                  <br>
                ...
                $this->load->view('loginassociadoempresafilial/tela_msg', $data);
                #redirect(base_url() . 'loginassociadoempresafilial' . $data['msg']);
                #exit();
            }
        }
    ...
    public function confirmar($codigo) {
        ...
        $id = $this->Loginassociadoempresafilial_model->get_data_by_codigo($codigo);

        if ($this->Loginassociadoempresafilial_model->ativa_usuario($codigo, $data['confirmar']) === TRUE) {
    ...
    public function recuperar() {
        ...
        if ($this->form_validation->run() === FALSE) {
            #load loginassociadoempresafilial view
            $this->load->view('loginassociadoempresafilial/form_recuperar', $data);
        } else {

            $data['query']['Codigo'] = md5(uniqid(time() . rand()));

            $id = $this->Loginassociadoempresafilial_model->get_data_by_usuario($data['query']['UsuarioEmpresaFilial']);

            if ($this->Loginassociadoempresafilial_model->troca_senha($id['idSis_EmpresaFilial'], array('Codigo' => $data['query']['Codigo'])) === FALSE) {
                ...
                $this->email->message('Por favor, clique no link a seguir para alterar sua senha: '
                        //. 'http://www.romati.com.br/app/loginassociadoempresafilial/trocar_senha/' . $data['query']['Codigo']);
                        . base_url() . 'loginassociadoempresafilial/trocar_senha/' . $data['query']['Codigo']);
    ...
    public function trocar_senha($codigo = NULL) {
        ...
        if ($codigo) {
            $data['query'] = $this->Loginassociadoempresafilial_model->get_data_by_codigo($codigo);
        ...
        if ($this->form_validation->run() === FALSE) {
            #load loginassociadoempresafilial view
            $this->load->view('loginassociadoempresafilial/form_troca_senha', $data);
        } else {
            ...
            if ($this->Loginassociadoempresafilial_model->troca_senha($data['query']['idSis_EmpresaFilial'], $data['query']) === TRUE) {
                $data['msg'] = '?m=2';
                $this->load->view('loginassociadoempresafilial/form_troca_senha', $data);
            } else {
    ...
    public function sair($m = TRUE) {
        ...
        if ($_SESSION['log'] && $m === TRUE) {
            $this->Loginassociadoempresafilial_model->set_acesso($_SESSION['log']['id'], 'LOGOUT');
        } else {
            if (!isset($_SESSION['log']['id'])) {
                $_SESSION['log']['id'] = 1;
            }
            $this->Loginassociadoempresafilial_model->set_acesso($_SESSION['log']['id'], 'TIMEOUT');
            $data['msg'] = '?m=2';
        }
        ...
        /*
          ...
          $this->load->view('loginassociadoempresafilial');
          $this->load->view('basico/footer');
         *
         */

        redirect(base_url() . 'loginassociadoempresafilial/' . $data['msg']);
        #redirect('loginassociadoempresafilial');
    }

    function valid_usuario($data) {

        if ($this->Loginassociadoempresafilial_model->check_usuario($data) == 1) {
            $this->form_validation->set_message('valid_usuario', '<strong>%s</strong> não existe.');
            return FALSE;
        } else if ($this->Loginassociadoempresafilial_model->check_usuario($data) == 2) {
    ...
    function valid_senha($senha, $usuario) {
        ...
        if ($this->Loginassociadoempresafilial_model->check_dados_usuario($senha, $usuario) == FALSE) {

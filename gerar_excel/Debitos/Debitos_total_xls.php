...
<?php if(isset($_SESSION['log']['idSis_Empresa']) && isset($_SESSION['FiltroDebitos'])) { ?>
				...
				while($row = mysqli_fetch_assoc($resultado_msg_contatos)){
					...
					$html .= '<tr>';
						...
						$html .= '<td>'.$row['DataOrca'] . '</td>';
							...
							$html .= '<td>'.$row['idApp_Fornecedor'] . '</td>';

						$html .= '<td>'.utf8_encode($row["NomeFornecedor"]).'</td>';
						$html .= '<td>'.$row['CelularFornecedor'] . ' / '.$row['Telefone2'] . ' / '.$row['Telefone3'] . '</td>';
